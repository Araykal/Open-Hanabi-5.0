...
import life.hanabi.utils.render.GuiBlock;
...
public class CustomKey extends AbstractKey {
    ...
    public void renderKey(int x, int y) {
        ...
        String name = this.type == 0 ? (!((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).chroma.getValue() ? EnumChatFormatting.STRIKETHROUGH.toString() + "-----" : "------") : this.getKeyOrMouseName(this.key);
        ...
        int color;
        if (pressed) {
            color = Math.min(255, (int) (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).fadeTime.getValue().doubleValue() * 5.0D * (double) (System.currentTimeMillis() - this.lastPress)));
            textBrightness = Math.max(0.0D, 1.0D - (double) (System.currentTimeMillis() - this.lastPress) / (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).fadeTime.getValue().doubleValue() * 2.0D));
        } else {
            color = Math.max(0, 255 - (int) (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).fadeTime.getValue().doubleValue() * 5.0D * (double) (System.currentTimeMillis() - this.lastPress)));
            textBrightness = Math.min(1.0D, (double) (System.currentTimeMillis() - this.lastPress) / (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).fadeTime.getValue().doubleValue() * 2.0D));
        }
        ...
        if (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).keyBackground.getValue()) {
            Gui.drawRect(left, top, right, bottom, ((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).backgroundColor.getColor());
        }
        ...
        if (((life.hanabi.modules.render.KeyStrokes) Hanabi.INSTANCE.moduleManager.modules.get("KeyStrokes")).chroma.getValue()) {
            if (this.type == 0) {
                ...
                this.drawGradientRect(0, 0, 2, 35, Color.HSBtoRGB((float) ((System.currentTimeMillis() - (long) (xIn * 10) - (long) (y1 * 10)) % 2000L) / 2000.0F, 0.8F, 0.8F), Color.HSBtoRGB((float) ((System.currentTimeMillis() - (long) ((xIn + 35) * 10) - (long) (y1 * 10)) % 2000L) / 2000.0F, 0.8F, 0.8F));
