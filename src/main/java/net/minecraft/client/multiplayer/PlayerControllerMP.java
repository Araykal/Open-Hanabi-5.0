...
import life.hanabi.modules.render.MoreParticles;
...
public class PlayerControllerMP {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        if (Hanabi.INSTANCE.moduleManager.modules.get("MoreParticles").stage) {
            for (int i = 0; i < ((MoreParticles) Hanabi.INSTANCE.moduleManager.modules.get("MoreParticles")).critParticles.getValue().byteValue(); i++) {
                mc.effectRenderer.emitParticleAtEntity(targetEntity, EnumParticleTypes.CRIT);
            }

            for (int i = 0; i < ((MoreParticles) Hanabi.INSTANCE.moduleManager.modules.get("MoreParticles")).sharpParticles.getValue().byteValue(); i++) {
                mc.effectRenderer.emitParticleAtEntity(targetEntity, EnumParticleTypes.CRIT_MAGIC);
            }
            Random rand = new Random();

            for (int i = 0; i < ((MoreParticles) Hanabi.INSTANCE.moduleManager.modules.get("MoreParticles")).lavaParticles.getValue().byteValue(); i++) {
                mc.theWorld.spawnParticle(EnumParticleTypes.BLOCK_CRACK, targetEntity.posX + ((double) rand.nextFloat() - 0.5D), targetEntity.posY + rand.nextFloat() + 1, targetEntity.posZ + ((double) rand.nextFloat() - 0.5D), -rand.nextFloat() * 2, rand.nextFloat(), rand.nextFloat() * 2, 152);
                mc.theWorld.spawnParticle(EnumParticleTypes.BLOCK_CRACK, targetEntity.posX + ((double) rand.nextFloat() - 0.5D), targetEntity.posY + rand.nextFloat() + 1, targetEntity.posZ + ((double) rand.nextFloat() - 0.5D), rand.nextFloat() * 2, rand.nextFloat(), -rand.nextFloat() * 2, 152);
                mc.theWorld.spawnParticle(EnumParticleTypes.BLOCK_CRACK, targetEntity.posX + ((double) rand.nextFloat() - 0.5D), targetEntity.posY + rand.nextFloat() + 1, targetEntity.posZ + ((double) rand.nextFloat() - 0.5D), -rand.nextFloat() * 2, rand.nextFloat(), rand.nextFloat() * 2, 152);
                mc.theWorld.spawnParticle(EnumParticleTypes.BLOCK_CRACK, targetEntity.posX + ((double) rand.nextFloat() - 0.5D), targetEntity.posY + rand.nextFloat() + 1, targetEntity.posZ + ((double) rand.nextFloat() - 0.5D), -rand.nextFloat() * 2, rand.nextFloat(), -rand.nextFloat() * 2, 152);
            }
        ...
        if (((MoreParticles) Hanabi.INSTANCE.moduleManager.modules.get("MoreParticles")).sound.getValue()) {
    ...
    public void setCurBlockDamageMP(float v) {
        curBlockDamageMP = v;
    }
}
